...
export default function ResetPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    setError(null)
    if (newPassword.length < 8) { setError(t('authPage.reset.errShort')); return }
    if (newPassword !== confirmPassword) { setError(t('authPage.reset.errMatch')); return }
    setStatus('loading')
    try {
  ...
  return (
    <PageWrapper maxWidth={480} padTop="clamp(32px, 6vw, 64px)">
      ...
      <h1 className="font-headline font-bold text-5xl md:text-6xl leading-tight tracking-normal text-hai-plum mb-3">
        {t('authPage.reset.heading')}<span className="text-hai-teal">.</span>
      </h1>
      <p className="text-base text-neutral-600 leading-relaxed mb-8 max-w-md">{t('authPage.reset.sub')}</p>
      ...
      <div className="bg-white rounded-[2rem] shadow-[0_30px_80px_-30px_rgba(54,33,62,0.2)] border border-neutral-100 p-6 md:p-8">
        ...
        <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-5">
          ...
          >
            {status === 'loading' ? (
              <><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />{t('authPage.reset.submitting')}</>
            ) : t('authPage.reset.submit')}
          </button>
        </form>
      </div>
